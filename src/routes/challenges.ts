...
router.get("/overview", async (req, res) => {
  ...
  if (dailyCount === 0) {
    ...
    const templates = [
      { title: "No Plastic Straw", desc: "Tolak sedotan plastik saat jajan hari ini", target: 1, unit: "hari", points: 50 },
      { title: "Bawa Tumbler", desc: "Bawa botol minum sendiri saat bepergian", target: 1, unit: "hari", points: 60 },
      { title: "Eco-friendly Commute", desc: "Gunakan transportasi umum hari ini", target: 1, unit: "hari", points: 100 },
      { title: "Zero Waste Meal", desc: "Habiskan makanan tanpa sisa", target: 1, unit: "hari", points: 40 },
      { title: "Hemat Listrik", desc: "Matikan lampu yang tidak terpakai selama 4 jam", target: 4, unit: "jam", points: 30 },
    ];
...
router.post("/:id/progress", async (req, res) => {
  ...
  if (newProgress >= userChallenge.challenge.targetValue && status !== "completed") {
    status = "completed";
    
    const updatedUser = await prisma.user.update({
      where: { id: userId },
      data: { totalPoints: { increment: userChallenge.challenge.rewardPoints } },
    });
